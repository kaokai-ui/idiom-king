...
export function useIdiomChainV2({
  ...
  }, [challengeLevelsResolved, mode, activeLevel, isChallengeMode]);

  const { state, dispatch, boardRef, loadLevel, lastSeedRef } = useIdiomV2ChainState(getLevelData, {
  ...
  const initialLoadKey = useMemo(
    () => `${mode}:${effectiveInitialLevelNumber}:${initialSeed ?? 'auto'}:${challengeLevelsResolved?.length ?? 0}:${sessionKey}:${activeLevel}`,
    [challengeLevelsResolved?.length, effectiveInitialLevelNumber, initialSeed, mode, sessionKey, activeLevel],
  );
  ...
  useEffect(() => {
    if (mode === 'challenge' && (!challengeLevelsResolved || challengeLevelsResolved.length === 0)) {
      return;
    }
    ...
    loadLevel(effectiveInitialLevelNumber, initialSeed ?? undefined);
  }, [challengeLevelsResolved, effectiveInitialLevelNumber, initialLoadKey, initialSeed, loadLevel, mode]);
  ...
  useEffect(() => {
    ...
    effectiveOnLevelComplete?.(state.levelNumber);
  }, [effectiveOnLevelComplete, mode, state.level, state.levelNumber, state.phase]);

  const hasNextLevel = effectiveMaxLevelNumber === undefined || state.levelNumber < effectiveMaxLevelNumber;

  const goToNextLevel = useCallback(() => {
    const nextLevelNumber = effectiveMaxLevelNumber === undefined
      ? state.levelNumber + 1
      : Math.min(effectiveMaxLevelNumber, state.levelNumber + 1);
    if (nextLevelNumber === state.levelNumber && effectiveMaxLevelNumber !== undefined) {
      return;
    }
    loadLevel(nextLevelNumber);
  }, [effectiveMaxLevelNumber, loadLevel, state.levelNumber]);
  ...
  const currentSeed = isChallengeMode
    ? (challengeLevelsResolved?.[state.levelNumber - 1]?.seed ?? null)
    : state.currentSeed;
