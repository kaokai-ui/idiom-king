...
/**
 * Shared crossword-style level generator used by both the v1 (`levelGenerator`)
 * and v2 (`levelGeneratorV2`) entry points. The only thing that differs between
 * the two versions is where the idiom pool comes from and its concrete type, so
 * the core is parameterized over a minimal `GeneratorIdiom` shape.
 */
export type GeneratorIdiom = {
  id: string;
  text: string;
  chars: string[];
};
