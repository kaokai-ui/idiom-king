...
function chainV2Reducer(state: ChainV2State, action: ChainV2Action): ChainV2State {
  switch (action.type) {
...
export function useIdiomV2ChainState(
  ...
  options: UseChainV2StateOptions,
) {
  const [state, dispatch] = useReducer(chainV2Reducer, initialState);
